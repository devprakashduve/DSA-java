...
public class Subarray_practice {
    public static void main(String[] args) {
        /*Find the contiguous non-empty subarray within an array, A of length N, with the largest sum.*/
//       int[] A = {-163, -20};
//        int A[]=  { -307, -316, -236, -136, -323, -370, -386 };
       int[] A =  { 56, -451, -222, 80, -57, -85, -304, -108, 95, -377, -282, -260, -497, -471, -97, -350, -147, 62, -447, -229, 78, -165, -333, -427, -59, -338, 60, -207, -247, -431, -54, -240, -426, -123, -209, 57, -74, -287, -70, -292, 45, -278, -408, -129, -35, 65, -77, -230, -426, 20, -402, -375, -449, -280, -125, -15, -21, -327, -366, 64, -211, -414, -307 };
...
//                System.out.println(A[i]);
                if (tempRes + A[i]>tempRes && tempRes>res){
//                    System.out.println(A[i]);
                    res=tempRes;
//                    break;
                }else if(tempRes + A[i]<=0 && tempRes>res){
                    System.out.println(A[i]);
                    res=tempRes;
                }
            }
    ...
    public static int maxSubArray2(final List<Integer> A) {
        int N = A.size();
        int maxSum = -1000; //From constraint
        int currSum = 0;
        boolean positive = false;

        for (int i = 0; i < N; i++) {
            if (!positive && A.get(i) > 0) positive = true;


            if (positive) {
                currSum += A.get(i);
                if (currSum < 0) {
                    currSum = 0; //Discard currSum when it becomes zero. From 1)
                }
                maxSum = Math.max(currSum, maxSum);
            }
            else {
                maxSum = Math.max(maxSum, A.get(i));//From 2)
            }
        }
